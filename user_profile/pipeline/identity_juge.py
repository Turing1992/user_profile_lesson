...
class Drawing():
    def __init__(self):
        ...
        self.identity_bloom_key = config["redis"]["identity_bloom_key_ems"]
            ...
            for data in datas:
                ...
                if idx in results_lookup:
                    ...
                    if data['content']!="":
                        ...
                        three_news = get_kind2(content)
                    mylog_hd.info(f"快递员身份判断{three_news}")
...
class MyListener(MessageListenerConcurrently):
    def __init__(self):
        ...
        self.md = Matcher()
        # self.md.load_from_collection(["快递员","快递小哥","送快递","快递老哥","快递车","丰巢快递柜","快递停运焦虑症","日常送货"])
        self.md.load_from_collection(["#快递"])
...
if __name__ == "__main__":
    ...
    consumer = DefaultMQPushConsumer(config["consumer_group"]["user_graph_uniq_user_identity2"])
