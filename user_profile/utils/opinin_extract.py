...
基于火山引擎（豆包）和 DeepSeek 大模型 API，提供多种身份识别和观点抽取功能，
包括外卖员、网约车司机、货车司机、快递员等特定职业身份的判断，
以及 KOL 账号的企业级分析（垂类、风格、内容质量等）。
"""
import json
import traceback
import os
...
def get_kind(txt):
    # type: (str) -> dict
    """基于外卖员身份判断 prompt 识别帖文发布者是否为外卖员。

    调用火山引擎（豆包）API，使用 promts3 作为系统提示词，
    判断帖文发布者是否为外卖员身份。
    ...
    Returns:
        包含 identity、identity2、log 字段的字典。
        识别失败时返回各字段为空字符串的字典。
    """
    headers = {
...
def get_kind2(txt):
    # type: (str) -> dict
    """基于快递员身份判断 prompt 识别帖文发布者是否为快递员。

    调用火山引擎（豆包）API，使用 promts6 作为系统提示词，
    判断帖文发布者是否为快递员身份。
...
def get_kind_wangyueche(txt):
    # type: (str) -> dict
    """基于网约车司机身份判断 prompt 识别帖文发布者是否为网约车司机。

    调用火山引擎（豆包）API，使用 promts4 作为系统提示词，
    判断帖文发布者是否为网约车司机身份。
...
def get_kind_huoche(txt):
    # type: (str) -> dict
    """基于货车司机身份判断 prompt 识别帖文发布者是否为货车司机。

    调用火山引擎（豆包）API，使用 promts5 作为系统提示词，
    判断帖文发布者是否为货车司机身份。
...
def identity_auto(promts,txt):
    ...
    调用火山引擎（豆包）API，使用自定义 prompt 作为系统提示词，
    对帖文内容进行身份识别。可配合不同的 prompt 实现多种身份判断。
...
def qiye_expect(txt):
    # type: (str) -> dict
    """调用 DeepSeek 大模型进行 KOL 企业级账号分析。

    对 KOL 账号的发帖信息进行综合分析，包括垂类判断、账号定位、
    文章风格、内容质量评估、品牌拉踩识别等。
    ...
    Returns:
        包含 industry、account_location、content_style、
        content_quelity、past_brands 字段的字典。
        识别失败时返回 {"result": ""}。
    """
    api_key = "sk-POKFI9rdc2olh-HduAH_kw"
    # base_url = "https://llmapi.paratera.com/v1/chat/completions"
