# -*- coding: utf-8 -*-
"""
Oasis系统 Web 测试服务 V2 (Apple UI)
端口 5002，服务 web_app_apple.html
"""
import json
import os
import uuid
...
@app.route('/')
def index():
    with open('web_app_apple.html', 'r', encoding='utf-8') as f:
...
def health_check():
    return jsonify({"status": "ok", "service": "Oasis Web Platform V2"})
    ...
    try:
        ...
        def run_sim():
            try:
                ...
                output_file = f"sim_v2_{task_id}_results.json"
...
def run_pipeline():
    try:
        ...
        task_id = str(uuid.uuid4())[:8]
        pipeline_tasks[task_id] = {"status": "running", "result": None, "error": None, "progress": "⏳ 初始化中...", "partial_result": None}

        def run():
            try:
                def on_progress(msg, partial=None):
                    pipeline_tasks[task_id]["progress"] = msg
                    if partial is not None:
                        pipeline_tasks[task_id]["partial_result"] = partial
                result = pipeline.run(event_data=data, platform=platform,
                                      sim_steps=sim_steps, use_llm_sim=use_llm_sim, progress_cb=on_progress)
                pipeline_tasks[task_id]["status"] = "completed"
                pipeline_tasks[task_id]["result"] = result
                pipeline_tasks[task_id]["partial_result"] = None  # 清理
            except Exception as e:
                traceback.print_exc()
...
def pipeline_status(task_id):
    ...
    if task["status"] == "running":
        ...
        if task.get("partial_result"):
            resp["partial_result"] = task["partial_result"]
    if task["status"] == "completed":
        resp["result"] = task["result"]
    elif task["status"] == "failed":
        resp["error"] = task["error"]
        ...
        def run():
            try:
                ...
                result = pipeline.run_from_keyword(
                    keyword=keyword, platform=platform, sim_steps=sim_steps,
                    use_llm_sim=use_llm_sim, start_time=start_time, end_time=end_time,
                    download_size=download_size, progress_cb=on_progress,
                    is_expression=is_expression)
                pipeline_tasks[task_id]["status"] = "completed"
                pipeline_tasks[task_id]["result"] = result
                pipeline_tasks[task_id]["partial_result"] = None
            except Exception as e:
                traceback.print_exc()
                pipeline_tasks[task_id]["status"] = "failed"
                pipeline_tasks[task_id]["error"] = str(e)
...
def get_example(name):
    mapping = {"profile": "example_input.json", "simulation": "example_profiles.json", "event": "example_events.json"}
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 50)
    print("  OASIS Web V2 (Apple UI) 已启动")
